...
const SessionCard = ({ session, onOpen }) => {
  ...
  if (session.status === "Approved") {
    ...
    scPill = "bg-gray-50 text-gray-500 border-gray-200";
    scDot = "bg-gray-400";
      ...
      <div className="flex items-center">
        ...
        <div className="flex flex-col gap-3 px-5 py-4 w-[380px] flex-shrink-0">
          <div className="flex justify-center">
            <span
              className={`flex items-center justify-center gap-1.5 text-[10px] font-black px-4 py-1.5 rounded-full border uppercase tracking-wider w-full ${scPill}`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${scDot}`}
              />
