...
const MoodAnalysisCard = () => {
  ...
  const minScore = Math.min(...Object.values(analysis.categoryAverages));
  const weakestCategories = Object.entries(analysis.categoryAverages)
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white border border-[#DBEAFE] px-6 py-5 flex items-center gap-5">
          <div
            className={`w-12 h-12 ${averageColors.bg} border ${averageColors.border} flex items-center justify-center shrink-0`}
          >
            <Activity
              size={22}
              className={averageColors.icon}
              strokeWidth={2}
            />
          </div>
          <div>
            ...
            <div className="flex items-baseline gap-1.5 mt-1 flex-wrap">
              <p
                className={`text-[28px] font-bold leading-none ${averageColors.text}`}
              >
                {analysis.weightedAverage}%
              </p>
              <span className={`text-[18px] font-bold ${averageColors.text}`}>
                — {getMoodLabel(analysis.weightedAverage)}
              </span>
            </div>
            <p className="text-[12px] text-[#6B7280] mt-1.5">
              Based on your last {analysis.totalQuizzes} quizzes
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#DBEAFE] px-6 py-5 flex items-center gap-5">
          <div
            className={`w-12 h-12 ${trendColors.bg} border ${trendColors.border} flex items-center justify-center shrink-0`}
          >
          ...
          <div>
            ...
            <p
              className={`text-[28px] font-bold leading-none mt-1 ${trendColors.text}`}
            >
              {analysis.trend}
            </p>
            <p className="text-[12px] text-[#6B7280] mt-1.5">
            ...
            {weakestCategories.length > 0 ? (
              <>
                <div className="flex items-baseline gap-2 mt-1 flex-wrap">
                  <p className="text-[28px] font-bold leading-none text-orange-500">
                    {weakestCategories[0]}
                  </p>
                  {weakestCategories.length > 1 && (
                    <span className="text-[18px] font-bold text-orange-400">
                      +{weakestCategories.length - 1} more
                    </span>
                  )}
                </div>
                <button
              ...
              return (
                <div key={name} className="flex flex-col gap-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <p
                      className={
                        "text-[13px] font-bold tabular-nums " + scoreColors.text
                      }
                    >
                      {score}/5
                    </p>
                  ...
                  <div className="w-full bg-[#F1F5F9] h-1.5">
                    <div
                      className={
                        scoreColors.bar + " h-1.5 transition-all duration-500"
                      }
